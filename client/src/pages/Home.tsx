    ...
    <Layout>
      ...
      <section className="py-24 bg-background relative overflow-hidden">
        <div className="absolute top-0 right-0 w-1/3 h-full opacity-5 pointer-events-none">
           <img src={fernTexture} className="w-full h-full object-cover mix-blend-multiply" />
        </div>
      ...
      <section className="py-24 bg-white">
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { icon: Activity, title: "Injury Recovery", desc: "Targeted treatment to help you heal from sports injuries or accidents." },
              { icon: Leaf, title: "Stress Relief", desc: "Calming techniques to lower cortisol and restore mental clarity." },
              { icon: Heart, title: "Pain Management", desc: "Evidence-based approaches to manage chronic or acute pain." },
              { icon: Droplets, title: "Deep Relaxation", desc: "Restorative sessions to help your nervous system reset." }
            ].map((feature, idx) => (
              <motion.div
                key={idx}
                initial={{ opacity: 0, y: 20 }}
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div className="max-w-2xl">
              <h2 className="text-3xl md:text-4xl font-serif mb-4 text-white">Therapeutic Services</h2>
              <p className="text-white/80 font-light text-lg">
                Whether you need deep tissue work or a moment of stillness, our treatments are tailored to your body's needs.
              </p>
            </div>
            <Link href="/services">
              <Button variant="outline" className="border-white/30 text-white hover:bg-white hover:text-primary rounded-full px-6 bg-transparent">
                View All Services
              </Button>
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={idx} className="group relative overflow-hidden rounded-xl aspect-[4/5] md:aspect-[3/4] bg-black/20 cursor-pointer">
                ...
                {/* Placeholder for service image if we had distinct ones, using hero/texture for now */}
                <img 
